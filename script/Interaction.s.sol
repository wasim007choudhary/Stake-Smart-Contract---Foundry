...
contract CreateSub is Script {
    ...
    function createSub(address vrfcoordinator, address account) public returns (uint256, address) {
...
contract fundSubscription is Script, CodeConstantVariable {
    ...
    function fundSubUsingConfig() public {
        ...
        if (subId == 0) {
            ...
            console.log("New subID create:", subId, "VRF address:", vrfcoordinator);
        }
        fundSub(vrfcoordinator, subId, link, account);
    }

    function fundSub(address vrfcoordinator, uint256 subId, address link, address account) public {
        ...
        if (block.chainid == LOCAL_CHAINID) {
            ...
            VRFCoordinatorV2_5Mock(vrfcoordinator).fundSubscription(subId, AMOUNT_TO_BE_FUNDED);
            ...
            vm.startBroadcast(account);
            LinkToken(link).transferAndCall(vrfcoordinator, AMOUNT_TO_BE_FUNDED, abi.encode(subId));
            vm.stopBroadcast();
        }
    }
...
contract AddConsumer is Script {
    ...
    function run() external {
        address deployedMostRecently = DevOpsTools.get_most_recent_deployment("Stake", block.chainid);
